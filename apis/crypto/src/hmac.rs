...
pub struct Hmac<S: Syscalls>(S);

impl<S: Syscalls> Hmac<S> {
    ...
    pub fn allow_key_buffer<'share>(
        buf: &'share [u8],
        allow_ro: share::Handle<AllowRo<'share, S, DRIVER_NUM, { ro_allow::KEY_BUF }>>,
    ) -> Result<(), ErrorCode> {
        S::allow_ro::<DefaultConfig, DRIVER_NUM, { ro_allow::KEY_BUF }>(allow_ro, buf)
    }

    pub fn unallow_key_buffer() {
        S::unallow_ro(DRIVER_NUM, ro_allow::KEY_BUF)
    }
    ...
    pub fn compute_sync(
        ...
        share::scope::<
            (
                AllowRo<S, DRIVER_NUM, { ro_allow::KEY_BUF }>,
            ...
            _,
        >(|handle| {
            let (allow_ro_key, allow_ro_input, allow_rw, subscribe) = handle.split();

            Self::allow_key_buffer(key_buf, allow_ro_key)?;
            Self::allow_input_buffer(input_buf, allow_ro_input)?;
...
pub struct HmacListener<F: Fn(u32)>(pub F);

impl<F: Fn(u32)> Upcall<OneId<DRIVER_NUM, 0>> for HmacListener<F> {
...
// ---------------
mod ro_allow {
    pub const KEY_BUF: u32 = 0;
    pub const INPUT_BUF: u32 = 1;
}
